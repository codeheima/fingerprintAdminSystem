...
// create private copy of $CTJ's String.format() method
...
Date.formatCodeToRegex = function(character, currentGroup) {
    ...
    return p ? $CTJ.applyIf({
      c: p.c ? xf(p.c, currentGroup || "{0}") : p.c
    }, p) : {
        g:0,
        c:null,
        s:$CTJ.escapeRe(character) // treat unrecognised characters as literals
    };
};
...
$CTJ.apply(Date, {
     ...
     * date formats are requested for $CTJ standard formatting strings.</p>
...
dt = Date.parseDate("2006-02-29 03:20:01", "Y-m-d H:i:s", true); // returns null
    ...
    parseDate : function(input, format, strict) {
        ...
        return p[format](input, $CTJ.isDefined(strict) ? strict : Date.useStrict);
    },
    // private
        ...
        var code = [
            ...
            "if(results){",
                ...
                "}else{",
                    ...
                    // date calculations (note: these calculations create a dependency on $CTJ.num())
                    "y = $CTJ.num(y, $CTJ.num(def.y, dt.getFullYear()));",
                    "m = $CTJ.num(m, $CTJ.num(def.m - 1, dt.getMonth()));",
                    "d = $CTJ.num(d, $CTJ.num(def.d, dt.getDate()));",
                    // time calculations (note: these calculations create a dependency on $CTJ.num())
                    "h  = $CTJ.num(h, $CTJ.num(def.h, dt.getHours()));",
                    "i  = $CTJ.num(i, $CTJ.num(def.i, dt.getMinutes()));",
                    "s  = $CTJ.num(s, $CTJ.num(def.s, dt.getSeconds()));",
                    "ms = $CTJ.num(ms, $CTJ.num(def.ms, dt.getMilliseconds()));",
                    "if(z >= 0 && y >= 0){",
...
// private
if ($CTJ.isSafari && (navigator.userAgent.match(/WebKit\/(\d+)/)[1] || NaN) < 420) {
    $CTJ.apply(Date.prototype, {
...
*/
/**
 * @class $CTJ.util.Format
 * Reusable data formatting functions
 * @singleton
 */
$CTJ.util.Format = function() {
    var trimRe         = /^\s+|\s+$/g,
        stripTagsRE    = /<\/?[^>]+>/gi,
    ...
    return {
        ...
        dateRenderer : function(format) {
            return function(v) {
                return $CTJ.util.Format.date(v, format);
            };
        },
        /**
        ...
        numberRenderer : function(format) {
            return function(v) {
                return $CTJ.util.Format.number(v, format);
            };
        },
        /**
 ...
 */
$CTJ.XTemplate = function(){
    $CTJ.XTemplate.superclass.constructor.apply(this, arguments);
    var me = this,
        s = me.html,
    ...
    while((m = s.match(re))){
       ...
       if (m3) {
           ...
           if(exp){
               fn = new Function(VALUES, PARENT, XINDEX, XCOUNT, WITHVALUES + RETURN +($CTJ.util.Format.htmlDecode(exp))+'; }');
           }
       }
       if (m4) {
           exp = m4 && m4[1] ? m4[1] : null;
           if(exp){
               exec = new Function(VALUES, PARENT, XINDEX, XCOUNT, WITHVALUES +($CTJ.util.Format.htmlDecode(exp))+'; }');
           }
       }
       if(name){
           switch(name){
...
$CTJ.extend($CTJ.XTemplate, $CTJ.Template, {
    // private
    re : /\{([\w-\.\#]+)(?:\:([\w\.]*)(?:\((.*?)?\))?)?(\s?[\+\-\*\\]\s?[\d\.\+\-\*\\\(\)]+)?\}/g,
    // private
    codeRe : /\{\[((?:\\\]|.|\n)*?)\]\}/g,
 ...
 * @param {Object/Array} values The template values. Can be an array if your params are numeric (i.e. {0}) or an object (i.e. {foo: 'bar'})
 * @return {String} The HTML fragment
 * @member $CTJ.XTemplate
 * @method apply
 */
$CTJ.XTemplate.prototype.apply = $CTJ.XTemplate.prototype.applyTemplate;
/**
 ...
 * @static
 */
$CTJ.XTemplate.from = function(el){
    el = $CTJ.getDom(el);
    return new $CTJ.XTemplate(el.value || el.innerHTML);
};
/**
